...
namespace metalog
{
#define METALOG_DEFINE_JOIN_CONJUNCTION(X, Y, Z) \
    ...
    < \
            ...
            BOOST_PP_COMMA_IF(BOOST_PP_AND(X, Y)) \
            METALOG_VARIADIC_PARAMS(Y, rH) \
            BOOST_PP_COMMA_IF(BOOST_PP_AND(BOOST_PP_OR(X, Y), BOOST_PP_SUB(Z, 2))) \
...
#define METALOG_FORWARD_DEFINE_JOIN_CONJUNCTION(Z, Y, X) \
    METALOG_DEFINE_JOIN_CONJUNCTION(X, Y, 2) \
    METALOG_DEFINE_JOIN_CONJUNCTION(X, Y, METALOG_MAX_VARIADIC_ARGS)
...
#else
    METALOG_DEFINE_JOIN_CONJUNCTION(METALOG_MAX_VARIADIC_ARGS, METALOG_MAX_VARIADIC_ARGS, 2)
    METALOG_DEFINE_JOIN_CONJUNCTION(METALOG_MAX_VARIADIC_ARGS, METALOG_MAX_VARIADIC_ARGS, METALOG_MAX_VARIADIC_ARGS)
#endif
}
